...
1. **Confirm parameters**
   ...
   - **Archobs is required.** Before starting the debate phases, run archobs analysis (see [`archobs`](../archobs/SKILL.md)) to generate coupling data, risk hotspots, and boundary health metrics. If `.archobs/artifacts/file_metrics.parquet` already exists and its mtime is newer than the most recent commit (`git log -1 --format=%ct`), reuse it; otherwise regenerate. Use the archobs output to ground findings in measured data — especially for systemic risks, hotspot identification, and prioritization.

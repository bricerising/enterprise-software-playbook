-- Remove 'lobsters' from the source CHECK constraint on the events table.
...
-- Delete any existing lobsters rows before copying (they'd violate new CHECK)
DELETE FROM events WHERE source = 'lobsters';
...
-- Clean up lobsters checkpoints and health rows
DELETE FROM checkpoints WHERE source = 'lobsters';
DELETE FROM collector_health WHERE source = 'lobsters';

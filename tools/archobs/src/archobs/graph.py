...
def _run_codanna(repo_path: str | Path, args: list[str]) -> str:
    proc = run(["codanna", *args], cwd=repo_path, check=True, capture_output=True, text=True)
    return proc.stdout


def _ensure_codanna_index(repo_path: str | Path, files_df: pd.DataFrame) -> None:
    ...
    if not index_paths:
        return
    _run_codanna(repo_path, ["index", *index_paths, "--no-progress"])
...
def _codanna_search(repo_path: str | Path, query: str, limit: int) -> list[dict[str, object]]:
    if not query:
        return []

    def invoke(args: list[str]) -> list[dict[str, object]]:
        try:
            payload = _safe_load_json(_run_codanna(repo_path, args))
        except CalledProcessError as exc:
            stdout = exc.stdout or ""
            payload = _safe_load_json(stdout) if stdout.strip().startswith("{") else {}
        return _extract_codanna_hits(payload)

    hits = invoke(["mcp", "semantic_search_with_context", f"query:{query}", f"limit:{limit}", "--json"])
    if hits:
        return hits
    return invoke(["mcp", "search_symbols", f"query:{query}", f"limit:{limit}", "--json"])
...
def _save_codanna_cache(cache_path: str | Path | None, cache: dict[str, dict[str, object]]) -> None:
    ...
    target.write_text(json.dumps({"queries": cache}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def build_codanna_semantic_edges(
    ...
    _ensure_codanna_index(repo_path, files_df)
    ...
    if pending:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_map = {executor.submit(_codanna_search, repo_path, query, search_limit): query for query in pending}
    ...
    for row in analysis_df.sort_values("path").itertuples(index=False):
        ...
        if not hits:
            ...
            if fallback_query:
                ...
                if hits is None:
                    hits = _codanna_search(repo_path, fallback_query, search_limit)
...
def build_semantic_edge_set(
    ...
    if provider_used == "codanna":
        return build_codanna_semantic_edges(
            repo, files_df, analysis_df, graph_config, cache_path=cache_path,
        )
    if embeddings is not None:
        return build_semantic_edges(files_df["path"].tolist(), embeddings, graph_config)
    return pd.DataFrame(columns=["path_a", "path_b", "similarity", "w_sem"])
...
def build_graph_artifacts(
    ...
    *,
    ...
    sem_builder = semantic_edge_builder if semantic_edge_builder is not None else build_semantic_edge_set
